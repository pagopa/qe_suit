...
## 4. Configure the contract validator and declare the final API-call identity

For API contracts, configure the `InteropHttpContractValidator` (or the project-specific API contract validator bean) used by the test module.

For WEB contracts, configure the `WebBrowserContractValidator`.

For authenticated API contract tests, declare the final API-call identity explicitly through:
...
This is the identity that will be used by the API call generated by the contract test.

The contract validator is responsible for generating or executing the JUnit `DynamicTest` instances according to the declared contract scenarios.

### API contract authentication pattern

Use the validator in this order:
...
- the temporary session used by the payload supplier does not become the final API-call identity;
- the framework restores the user declared through `as(...)` immediately before the actual API call is executed;
- the generated `DynamicTest` instances are executed as the final identity declared in the DSL.
